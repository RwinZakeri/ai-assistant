...
interface DeleteDeliveryTimeModalProps {
  open: boolean;
...
const DeleteDeliveryTimeModal = ({
  open,
  onOpenChange,
  onConfirm,
  isLoading = false,
}: DeleteDeliveryTimeModalProps) => {
  const handleClose = () => {
    if (!isLoading) {
      onOpenChange(false);
    }
        ...
        <div className="flex flex-col items-start px-6 pt-[32px] pb-6 ">
          ...
          <div className="mt-3 gap-4 flex flex-col items-start">
            ...
            <p className="text-sm-regular text-textTertiary  leading-relaxed">
              آیا مطمئن هستید که می‌خواهید این بازه زمان را حذف کنید؟ این عمل
              قابل لغو نیست.
            </p>
          </div>
...
export default DeleteDeliveryTimeModal;
